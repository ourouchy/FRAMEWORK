(() => {
  ...
  var Collection = class {
    constructor(apiUrl2, deserialize) {
      this.apiUrl = apiUrl2;
    ...
    fetch() {
      axios_default.get(this.apiUrl).then((response) => {
  ...
  var UserForm = class extends View {
    ...
    onSetAgeClick = () => {
      ...
      console.log("Age g\xE9n\xE9r\xE9");
    };
    onSetNameClick = () => {
  ...
  var UserEdit = class extends View {
    ...
    attachViews() {
      const users2 = User.buildUserCollection();
      users2.fetch();
      users2.on("change", () => {
        new UserList(this.regions.userList, users2);
      });
  ...
  var UserList = class {
    ...
    render() {
      ...
      <select class="user-select">
        <option value="" disabled selected>-- S\xE9lectionnez un utilisateur --</option>
        ${this.users.models.map(
        (user) => `<option value="${user.get("id")}">${user.get("name")}</option>`
      ).join("")}
      </select>
      <button class="create-user">Cr\xE9er un nouvel utilisateur</button>
      <button class="refresh-list">Rafra\xEEchir pour mettre \xE0 jour la liste</button>
    </div>
    `;
      this.bindEvents();
    }
    bindEvents() {
      ...
      const createUserButton = this.parent.querySelector(".create-user");
      createUserButton.addEventListener("click", this.onCreateUser);
      const refreshButton = this.parent.querySelector(".refresh-list");
    ...
    onUserSelect = (event) => {
      ...
      if (selectedUser) {
        ...
        new UserEdit(rootElement2, selectedUser).render();
      }
    };
    onCreateUser = () => {
      const rootElement2 = document.getElementById("root");
      ...
      new UserEdit(rootElement2, newUser).render();
    };
  };

  // src/index.ts
  var rootElement = document.getElementById("root");
  var users = User.buildUserCollection();
  users.fetch();
  users.on("change", () => {
    new UserList(rootElement, users);
  });
  var defaultUser = User.buildUser({ name: "John Doe", age: 30 });
